...
stdout/stderr at `~/.sigidi/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.sigidi/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.sigidi/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.sigidi/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.sigidi/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${T3CODE_HOME:-$HOME/.sigidi}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.sigidi/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.sigidi/dev/logs/server.trace.ndjson"
```
