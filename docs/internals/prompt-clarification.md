...
The controller applies a result only when the request token, environment, draft key, text, and revision still match. Cancellation, send, disconnect, route or environment change, and unmount invalidate the local request. Cancellation does not promise to stop provider-native work. If the draft changes before a result returns, the panel enters the **Draft changed** state; it keeps the result separate and permits replacement only through the explicit action.
