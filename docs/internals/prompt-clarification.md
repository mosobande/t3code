...
Focused automated proof covers built-in slash parsing; request single-flight, cancellation, and stale-result handling; terminal-context placeholder removal and restoration; disabled-state reasons; keybinding schema and catalogue visibility; primary-environment settings and restore policy; each provider's Clarify isolation path; and the WebSocket RPC boundary. Browser proof covers the registered composer, command-palette, and Settings surfaces separately.
