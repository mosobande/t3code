...
pushes to the repository default branch:
...
`APPLE_TEAM_ID` and the channel-specific `MACOS_PROVISIONING_PROFILE_STABLE` or
`MACOS_PROVISIONING_PROFILE_NIGHTLY` secret. Windows uses Azure Trusted Signing. Without the core
signing credentials, it still releases unsigned artifacts.
