...
2. The target runs `__service-preflight`, verifies that the stable launcher supports its update
   protocol, and requires both the upstream and SIGIDI migration ledgers to match the target
   manifests exactly. A pending migration in either lane blocks remote replacement.
...
8. The trial runs the two-lane migration owner, acquires dependencies, binds HTTP, starts every
   long-running root fiber, and verifies that each root is parked at the activation gate. Preflight
   already required exact manifests, so the migration runner validates state without applying a
   pending migration.
...
server stops and before the trial starts. This makes trial writes reversible. The snapshot is
...
service. A later release can use the remote trial path only when neither migration lane is pending;
releases with migrations require this local update path.
