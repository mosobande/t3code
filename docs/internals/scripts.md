...
- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.sigidi`, even
  when `T3CODE_HOME` is set, storing state in `<worktree>/.sigidi/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.sigidi/dev`, keeping development state
  separate from `~/.sigidi/userdata`. An explicit `--home-dir <path>` stores state under
