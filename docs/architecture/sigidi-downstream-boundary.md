...
| [Prompt clarification](../internals/prompt-clarification.md) | Shared text-generation API and five provider adapters; composer, settings, command, contract, and server host files listed in the feature record | `a483337a` | Add the SIGIDI-owned pre-send rewrite operation and register its web and server entry points without a generic extension framework. | Prompt clarification maintainer | Focused contract, provider, service, RPC, controller, composer-logic, settings, and keybinding tests listed in the feature record. | Remove or narrow these edits when upstream exposes stable provider-neutral utility-generation and composer/settings command seams that preserve the accepted behavior. | — | `a483337a` |
