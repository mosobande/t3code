...
| [Prompt clarification](../internals/prompt-clarification.md) | Shared text-generation API and five provider adapters; composer toolbar and overflow, settings, command, contract, and server host files listed in the feature record | `a483337a` | Add the SIGIDI-owned pre-send rewrite operation, independent model selection, and web/desktop entry-point registration without a generic extension framework. | Prompt clarification maintainer | Required focused contract, provider, service, RPC, controller, composer, settings, and keybinding proof listed in the feature record. | Remove or narrow these edits when upstream exposes stable provider-neutral utility-generation and composer/settings command seams that preserve independent selection, strict availability, no-send behavior, explicit replacement, and stale-result protection. | — | `a483337a` |
