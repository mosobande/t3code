...
import { replaceTemporaryWorktreeBranchPrefix } from "@t3tools/shared/git";
...
const makeWsRpcLayer = (
  ...
  projectNotes: ProjectNoteStore["Service"],
) =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const promptClarification = yield* PromptClarification.PromptClarification;
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.promptClarificationRewrite]: (input) =>
          observeRpcEffect(
            WS_METHODS.promptClarificationRewrite,
            promptClarification.rewrite(currentSession.sessionId, input),
            { "rpc.aggregate": "sigidi.promptClarification" },
          ),
        ...
        [WS_METHODS.projectsGetNote]: (input) =>
          observeRpcEffect(WS_METHODS.projectsGetNote, projectNotes.get(input), {
            "rpc.aggregate": "workspace",
          }),
        [WS_METHODS.projectsUpdateNote]: (input) =>
          observeRpcEffect(WS_METHODS.projectsUpdateNote, projectNotes.update(input), {
            "rpc.aggregate": "workspace",
          }),
...
export const websocketRpcRouteLayer = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const serverSelfUpdate = yield* ServerSelfUpdate.ServerSelfUpdate;
    const projectNotes = yield* ProjectNoteStore;
    const pullRequests = yield* PullRequestService.PullRequestService;
    return HttpRouter.add(
      ...
      Effect.gen(function* () {
        ...
        const rpcWebSocketHttpEffect = yield* RpcServer.toHttpEffectWebsocket(WsRpcGroup, {
          ...
          Effect.provide(
            makeWsRpcLayer(session, previewAutomationBroker, projectNotes).pipe(
