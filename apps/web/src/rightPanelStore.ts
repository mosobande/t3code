...
// v12 removes the legacy Clarify panel; Clarify is now a composer action.
            ...
            .map(([threadKey, threadState]) => {
              ...
              const surfaces = Array.isArray(validThreadState?.surfaces)
                ? validThreadState.surfaces.flatMap<RightPanelSurface>((surface) => {
                    ...
                    // Dropped surface kind: Clarify now runs directly from
                    // the composer (v12), so stale panel state never returns.
                    if ((surface as { kind?: string }).kind === "clarify") return [];
              ...
              const persistedActiveSurfaceId = surfaces.some(
                ...
                : rawActiveSurfaceId === "pull-request"
                  ...
                  : null;
              // A migration that dropped every surface (for example a legacy
              // plan or Clarify-only panel) must not reopen an empty panel.
              const isOpen =
                surfaces.length > 0 &&
