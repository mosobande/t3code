...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx ${cliPackageName}@${targetVersion}`;
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the SIGIDI desktop app on its machine — update the desktop app there to sync them.`;
    default:
