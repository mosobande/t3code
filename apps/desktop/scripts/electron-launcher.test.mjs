...
describe("electron development launcher", () => {
  it("uses the SIGIDI development identity", () => {
    const identity = resolveLauncherIdentity({
      isDevelopment: true,
      devBundleIdSuffix: "worktree",
    });

    assert.equal(identity.displayName, "SIGIDI Dev");
    assert.equal(identity.bundleId, "com.quantipixels.sigidi.dev.worktree");
    assert.deepEqual(identity.protocolSchemes, ["sigidi-dev"]);
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/SIGIDI Dev.app",
      "SIGIDI Dev",
    );

    assert.equal(paths.launcherExecutableName, "SIGIDI Dev Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/SIGIDI Dev.app/Contents/MacOS/SIGIDI Dev Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/SIGIDI Dev.app/Contents/MacOS/Electron",
    );
