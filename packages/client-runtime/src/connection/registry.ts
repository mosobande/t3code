...
export const makeWithTargetPolicy = Effect.fn("EnvironmentRegistry.makeWithTargetPolicy")(
  function* (allowsTarget: ConnectionTargetPolicy) {
    ...
    const persistedTargets = (yield* storage.list).filter(allowsTarget);
    ...
    const entries =
      yield* SubscriptionRef.make<ReadonlyMap<EnvironmentId, ConnectionCatalogEntry>>(
        initialEntries,
      );
    ...
    const reconcilePlatform = Effect.fn("EnvironmentRegistry.reconcilePlatform")(function* (
      ...
      const allowedRegistrations = platformRegistrations.filter((registration) =>
        allowsTarget(registration.target),
      );
...
export const layerWithTargetPolicy = (allowsTarget: ConnectionTargetPolicy) =>
  Layer.effect(EnvironmentRegistry, makeWithTargetPolicy(allowsTarget));
