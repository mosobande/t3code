...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .sigidi > ambient T3CODE_HOME)
    ...
    it.effect("ignores an ambient T3CODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { T3CODE_HOME: "/home/user/custom-data" },
          serverOffset: 0,
          webOffset: 0,
          t3Home: undefined,
          browser: undefined,
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            ...
            cwd: root,
            ambientHome: "/home/user/custom-data",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".sigidi"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .sigidi over an ambient T3CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient T3CODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.resolve("/home/user/custom-data"));
        }),
      );
